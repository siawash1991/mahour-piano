...
test('curriculum has unique, valid exercises across six levels',()=>{assert.equal(new Set(allLessons.map(l=>l.id)).size,allLessons.length);assert.equal(new Set(lessons.map(l=>l.level)).size,6);assert.equal(songs.length,26);for(const l of allLessons){assert.ok(l.instructions.length>=3,l.id);assert.ok(l.steps.length>=4,l.id);for(const s of l.steps){assert.ok(s.midi>=36&&s.midi<=84);assert.ok(s.beats>0&&s.beats<=4)}}});
